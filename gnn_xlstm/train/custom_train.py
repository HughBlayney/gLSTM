...
from gnn_xlstm.loss.subtoken_prediction_loss import subtoken_cross_entropy
...
def custom_train(loggers, loaders, model, optimizer, scheduler):
    ...
    for cur_epoch in range(start_epoch, cfg.optim.max_epoch):
        ...
        if is_eval_epoch(cur_epoch):
            ...
            if cfg.optim.early_stopping_patience is not None:
                if int(cur_epoch) - int(best_epoch) > int(
                    cfg.optim.early_stopping_patience
                ):
                    logging.info(f"Early stopping at epoch {cur_epoch} due to patience")
                    break
            if cfg.optim.early_stopping_loss_threshold is not None:
                if perf[1][cur_epoch]["loss"] < cfg.optim.early_stopping_loss_threshold:
                    logging.info(
                        f"Early stopping at epoch {cur_epoch} due to loss threshold"
                    )
                    break
...
def log_attn_weights(loggers, loaders, model, optimizer=None, scheduler=None):
    ...
    from gnn_xlstm.utils import unbatch, unbatch_edge_index
