...
def normalize(node_labels, graph_labels):
    # normalize labels
    max_node_labels = torch.cat(
        [nls.max(0)[0].max(0)[0].unsqueeze(0) for nls in node_labels["train"]]
    ).max(0)[0]
    max_graph_labels = torch.cat(
        [gls.max(0)[0].unsqueeze(0) for gls in graph_labels["train"]]
    ).max(0)[0]
...
class GPPDataset(InMemoryDataset):
    ...
    def __init__(self, root, name, split="train", pre_transform=None, transform=None):
        ...
        self._processed_filename = f"{self.split}_{self.name}_pre_transform_{self.pre_transform.__class__.__name__ if self.pre_transform is not None else None}.pt"
        self._processed_dir = osp.join(root, self.name, "processed")
        self._raw_dir = osp.join(root, self.name, "raw")
    ...
    def process(self):
        (adj, features, node_labels, graph_labels) = torch.load(
            open(
                osp.join(self.raw_dir, self.raw_file_names, f"pna_dataset_25-35.pkl"),
                "rb",
            )
        )

        # Note - I have deliberately commented this out. This is because the original
        # released ADGN (GPP) code contains a subtle bug where pre_transform is set
        # after init, which means it's still None when process is called. Since in that
        # version of the code, pre_transform is used to apply the normalisation, that
        # means that normalisation is never applied.
        # Therefore, in order to maintain consistency with those results, I have _not_
        # applied normalisation here. I address this slightly more in the paper.
        # It's also worth noting that turning normalisation _on_ can change some of the
        # results quite significantly - GCN performs much better with normalisation on.
        ...
        for batch_id in range(n_batches):
            ...
            for sample_id in range(n_samples_in_batch):
                ...
                if self.name == "sssp":
                    ...
                    y = gl[1].unsqueeze(0).unsqueeze(1)
                else:
